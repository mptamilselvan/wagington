...
class CheckoutService
{
    ...
    /**
     * Wrapper to validate multiple vouchers via VoucherService.
     * Keeps consumers (controllers/livewire) dependent on CheckoutService only.
     *
     * @param array $codes
     * @param int|null $userId
     * @return array ['valid' => [...], 'errors' => [...]]
     */
    public function validateMultipleVouchers(array $codes, ?int $userId = null): array
    {
        $voucherService = app(VoucherService::class);
        return $voucherService->validateMultipleVouchers($codes, $userId ?? 0);
    }
    ...
    public function buildUnifiedSummary(
        ...
        $shippingAmount = 0.0;
        $regionCode = null;
        // Prepare tax context and calculate shipping if we have a shipping address
        if ($shippingAddress) {
    ...
    public function checkoutUnified(
        ...
        $discountAmount = 0.0; $appliedVouchers = [];
        $couponCodes = [];
        ...
        if (!empty($couponCodes)) {
            ...
            $regionCode = $shippingAddress ? $this->deriveRegionFromAddress([
                'country' => $shippingAddress->country,
                'state' => $shippingAddress->state ?? null,
                'region' => $shippingAddress->region ?? null,
                'postal_code' => $shippingAddress->postal_code,
            ]) : null;
            $taxCalc = $this->calculateCartTax(max(0.0, $subtotal - $discountAmount), ['region' => $regionCode]);
        }
    ...
    // Determine applied tax rate from tax calculation if available
    $appliedTaxRate = $taxCalc['rate'] ?? null;

    $order = $this->createPendingOrder($updatedCart, $user, $reservedMap, $hasBackorder, $subtotal, $discountAmount, $taxAmount, $totalAmount, $appliedVouchers, $appliedTaxRate);
        ...
        if ($user) {
            ...
            if ($paymentResult['success']) {
                ...
                try {
                    \Log::info('CheckoutService: Sending payment failed email', [
                        'order_number' => $order->order_number,
                        'user_id' => $user->id,
                    ...
                    // Send payment failed email
                    \Mail::to($user->email)->send(new \App\Mail\OrderPaymentFailed($order));
                } catch (\Exception $e) {
                    \Log::error('CheckoutService: Failed to send payment failed email', [
                        'order_number' => $order->order_number,
                        'user_id' => $user->id,
                        'error' => $e->getMessage(),
                    ]);
        ...
        try {
            \Log::info('CheckoutService: Sending order confirmation email', [
                'order_number' => $order->order_number,
                'user_id' => $user->id,
                'user_email' => $user->email,
            ]);
            ...
            \Log::error('CheckoutService: Failed to send order confirmation email', [
                'order_number' => $order->order_number,
                'user_id' => $user->id,
                'error' => $e->getMessage(),
            ]);
     ...
     */
    private function createPendingOrder(array $cart, ?\App\Models\User $user, array $reservedMap, bool $hasBackorder, float $subtotal, float $discountAmount, float $taxAmount, float $totalAmount, array $appliedVouchers = [], ?float $appliedTaxRate = null): \App\Models\Order
    {
        return DB::transaction(function () use ($cart, $user, $reservedMap, $hasBackorder, $subtotal, $discountAmount, $taxAmount, $totalAmount, $appliedVouchers, $appliedTaxRate) {
    ...
    private function attemptPaymentWithRetries(\App\Models\User $user, \App\Models\Order $order, float $totalAmount, int $maxRetries, ?string $paymentMethodId): array
    {
        ...
        while ($attempts < $maxRetries) {
            ...
            try {
                ...
                // Extract payment intent from result
                ...
                //increment the order's failed attempts counter
                $order->increment('payment_failed_attempts');
                \Log::warning('Payment attempt failed - incremented counter', [
                    'attempt' => $attempts,
                    'order_id' => $order->id,
                    'failed_attempts' => $order->payment_failed_attempts,
                    'error' => $lastError,
                ]);
    ...
    public function cartRequiresShipping(array $cart): bool
    {
        ...
        // Load variants and related product in a single query
        $variants = ProductVariant::with('product:id,shippable')
            ->whereIn('id', array_values($variantIds))
            ->get()
            ->keyBy('id');
