                ...
                <table class="min-w-full bg-white table-auto">
                    <thead class="">
                        <tr>
                            ...
                            <th class="text-sm font-normal text-left text-gray-500">Test name</th>
                    ...
                    <tbody>
                        @foreach ($data as $blood_test)
                            <tr class="border-t">
                                <td class="text-sm text-left text-gray-700">{{ $blood_test->species->name }}</td>
                                <td class="text-sm text-left text-gray-700">{{ $blood_test->name }}</td>
                                <td class="text-sm text-left text-gray-700">{{ $blood_test->expiry_days }}</td>
                                <td class="text-left">
                                    @component('components.checkbox-toggle', [
                                        'id' => 'is_active',
                                        'value' => $blood_test->is_active,
                                        'wireModel' => 'is_active.'.$blood_test->id,
                                        'checked' => $blood_test->is_active,
                                        'wireClickFn' => 'toggleStatus('.$blood_test->id.')',
                                        ])
                                    @endcomponent
                                </td>
                                <td class="td">
                                    @component('components.three-dots-trigger', [
                                        'menuItems' => [
                                            ['label' => 'Edit', 'wireFn' => "edit($blood_test->id)"],                                               
                                            ['label' => 'Delete', 'wireFn' => "deletePopUp($blood_test->id)"],
                                        ],
                                    ])
                                    @endcomponent 
                                    {{-- <button wire:click="edit({{ $blood_test->id }})" class="text-blue-600 hover:underline">Edit</button>
                                    <button wire:click="deletePopUp({{ $blood_test->id }})" class="text-red-600 hover:underline">Delete</button> --}}
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
