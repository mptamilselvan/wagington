<div class="min-h-screen bg-gray-50 lg:ml-72">
    @if ($showForm)
        <!-- Add/Edit Room Type Form - Full Page -->
        <div class="min-h-screen bg-gray-50">
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center">
                                ...
                                <div>
                                    ...
                                    @if($editingRoomType)
                                        <p class="text-sm text-gray-500 mt-1">Editing: {{ $editingRoomType->name }}</p>
                                    @endif
                                </div>
                            </div>
                        </div>
            ...
            <!-- Room Type Form Content -->
            <div class="px-4 sm:px-6 pb-8">
                <div class="bg-white rounded-lg shadow-sm border border-gray-200">
                    <div class="px-6 py-6">
                        {{-- Add/Edit Room Type Form --}}
                        @if($editingRoomType)
                            @include('components.room.edit-room-type', ['roomTypes' => $roomTypes, 'species' => $species, 'serviceCategories' => $serviceCategories, 'serviceAddons' => $serviceAddons])
                        @else
                            @include('components.room.add-room-type', ['roomTypes' => $roomTypes, 'species' => $species, 'serviceCategories' => $serviceCategories, 'serviceAddons' => $serviceAddons])
                        @endif
    ...
    @else
        <!-- Room Type List Page -->
        @include('components.room.room-type-list') 
    @endif
</div>