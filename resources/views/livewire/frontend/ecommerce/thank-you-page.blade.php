@if (!empty($order))
    @section('meta_tags')
        @php
            ...
            $metaDescription =
                'Your order has been successfully placed. Thank you for shopping with us. View your order details and track your shipment.';
            // Only set canonical URL if we have a valid order number
            $orderNumber = optional($order)->order_number;
            $canonicalUrl = $orderNumber
                ? route('shop.thank-you', ['orderNumber' => $orderNumber])
                : route('shop.thank-you'); // Fallback to base thank-you route
        @endphp
...
<x-ecommerce.page-wrapper heroImage="/images/e-comerce-1.png"> <x-slot name="header">
    ...
    @if ($order)
        ...
        <div class="mb-8">
            <div
                class="bg-gradient-to-r from-white via-green-50 to-white border border-gray-200 rounded-xl p-6 shadow-lg">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                    <div class="mb-4 sm:mb-0">
                        ...
                        <div
                            class="flex flex-col sm:flex-row sm:items-center sm:space-x-2 text-sm text-gray-600 space-y-1 sm:space-y-0">
                            <span>Your order</span>
                            <span class="hidden sm:inline text-gray-400">|</span>
                            <span>Order ID: <span
                                    class="font-mono font-medium text-gray-900 bg-gray-100 px-2 py-1 rounded">{{ $order->order_number }}</span></span>
                            <span class="hidden sm:inline text-gray-400">|</span>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="lg:col-span-8 space-y-6">
                <!-- Order Items (Catalog 1) -->
                <div class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                    <div class="flex items-center justify-between mb-6">
                        <h2 class="text-lg font-semibold text-gray-900">Total Products: {{ $order->items->count() }}
                        </h2>
                    </div>

                    <div class="space-y-4">
                        @foreach ($order->items as $item)
                            <x-ecommerce.order-item-card :item="$item" :productImage="$itemImages[$item->id] ?? null" :addonImages="$addonImages" />
                        @endforeach
                    </div>
                </div>

                @if (isset($roomBookings) && $roomBookings->count() > 0)
                    <!-- Room Bookings (Catalog 3) -->
                    <div class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                        <div class="flex items-center justify-between mb-6">
                            <h2 class="text-lg font-semibold text-gray-900">Room Bookings: {{ $roomBookings->count() }}
                            </h2>
                        </div>
                        ...
                        <div class="divide-y">
                            @foreach ($roomBookings as $booking)
                                @php
                                    ...
                                    $roomLabel = $booking->room?->name ?? ($booking->room?->room_number ?? 'Room');
                                    $checkIn = $booking->check_in_date;
                                    $checkOut = $booking->check_out_date;
                                    $price = number_format((float) $booking->total_price, 2);
                                    $speciesName = $booking->species?->name ?? null;

                                    // Get room type image
                                    $roomTypeImage = null;
                                    if ($booking->roomType) {
                                        $roomTypeImage = $booking->roomType->getPrimaryImageUrl();
                                        // Fallback to first image if no primary image
                                        if (
                                            !$roomTypeImage &&
                                            $booking->roomType->images &&
                                            is_array($booking->roomType->images) &&
                                            count($booking->roomType->images) > 0
                                        ) {
                                            $firstImage = $booking->roomType->images[0];
                                            if (is_array($firstImage) && isset($firstImage['url'])) {
                                                $roomTypeImage = $firstImage['url'];
                                            } elseif (is_string($firstImage)) {
                                                $roomTypeImage = $firstImage;
                                            }
                                        }
                                    }

                                    $petNames = [];
                                    if (is_array($booking->pets_reserved)) {
                                        $ids = [];
                                        $it = new \RecursiveIteratorIterator(
                                            new \RecursiveArrayIterator($booking->pets_reserved),
                                        );
                                        foreach ($it as $val) {
                                        ...
                                        if (!empty($ids)) {
                                            $loaded = \App\Models\Pet::whereIn('id', array_unique($ids))
                                                ->get()
                                                ->keyBy('id');
                                            foreach (array_unique($ids) as $pid) {
                                                if (isset($loaded[$pid])) {
                                                    $petNames[] = $loaded[$pid]->name;
                                                }
                                            }
                                        }
                                    }
                                ...
                                <div class="py-4 flex items-start space-x-4">
                                    ...
                                    <div class="flex-shrink-0 w-20 h-20 bg-gray-100 rounded-lg overflow-hidden">
                                        @if ($roomTypeImage)
                                            <img src="{{ $roomTypeImage }}" alt="{{ $roomTypeName }}"
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="text-sm font-medium text-gray-900">{{ $roomTypeName }}</div>
                                        <div class="text-sm text-gray-600 mt-1">
                                            @if ($speciesName)
                                                <span class="mr-2">Species: {{ $speciesName }}</span>
                                            @endif
                                            @if (!empty($petNames))
                                                <span class="mr-2">Pets: {{ implode(', ', $petNames) }}</span>
                                            @endif
                                        </div>
                                        <div class="text-sm text-gray-600 mt-1">Check-in: {{ $checkIn }} |
                                            Check-out:
                                            {{ $checkOut }}</div>
                                    </div>
                                    ...
                                    <div class="text-right flex-shrink-0">
                                        <div class="text-sm font-semibold text-gray-900">S${{ $price }}</div>
                                        <div class="text-xs text-gray-500 mt-1">{{ ucfirst($booking->booking_status) }}
                                            /
                    ...
                    <div class="p-6">
                        <div class="grid gap-4 md:grid-cols-2">
                            ...
                            @if ($order->shippingAddress)
                                <div
                                    class="border rounded-lg p-4 transition-all duration-200 border-gray-200 hover:border-gray-300">
                                    <div class="mb-3">
                                        <div class="flex items-start justify-between mb-2">
                                            <div class="flex-1 min-w-0">
                                                <div class="font-medium text-gray-900 text-sm">
                                                    {{ $order->shippingAddress->label ?? 'Address' }}</div>
                                                <div class="text-gray-600 text-xs space-y-0.5 mt-1">
                                                    ...
                                                    @endif
                                                    <div>{{ $order->shippingAddress->country }}
                                ...
                                <div
                                    ...
                                    <div class="mb-3">
                                        <div class="flex items-start justify-between mb-2">
                                            <div class="flex-1 min-w-0">
                                                <div class="font-medium text-gray-900 text-sm">
                                                    {{ $order->billingAddress->label ?? 'Address' }}</div>
                                                <div class="text-gray-600 text-xs space-y-0.5 mt-1">
                                                    ...
                                                    @endif
                                                    <div>{{ $order->billingAddress->country }}
                                                        {{ $order->billingAddress->postal_code }}</div>
                                                </div>
                                            </div>
                ...
                @if ($totalSavings > 0)
                    <div
                        class="bg-gradient-to-r from-green-50 via-emerald-50 to-green-50 border-2 border-green-200 rounded-xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 relative overflow-hidden">
                        <!-- Background decoration -->
                        <div
                            class="absolute top-0 right-0 w-32 h-32 bg-green-100 rounded-full opacity-20 transform translate-x-16 -translate-y-8">
                        </div>
                        <div
                            class="absolute bottom-0 left-0 w-20 h-20 bg-emerald-100 rounded-full opacity-30 transform -translate-x-8 translate-y-8">
                        </div>

                        <div class="relative flex items-center justify-between">
                            <div class="flex items-center space-x-4">
                                <div
                                    class="w-12 h-12 bg-gradient-to-br from-green-400 to-emerald-500 rounded-full flex items-center justify-center shadow-lg">
                ...
                <div class="lg:hidden bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                    ...
                    <div class="space-y-3 text-sm">
                        ...
                        @if ($mobileCouponDiscount > 0)
                            <div class="flex justify-between text-green-700">
                                <span class="flex items-center">
                                    ...
                                    Coupon @if (!empty($order->coupon_code))
                                        ({{ $order->coupon_code }})
                                    @endif
                                </span>
                        ...
                        <div class="border-t pt-3 flex justify-between text-base">
                            ...
                            <span
                                class="font-bold text-lg text-gray-900">${{ number_format($order->total_amount, 2) }}</span>
                        </div>
                    </div>
                </div>
                ...
                @if ($hasInvoice)
                    <x-ecommerce.invoice-section :payment="$payment" :order="$order"
                        wrapperClass="mt-6 pt-6 border-t border-gray-200 lg:hidden" />
                @endif
            </div>

            <!-- Right Column - Order Actions (Desktop Only) -->
            <div class="hidden lg:block lg:col-span-4">
                <x-ecommerce.order-actions :order="$order" :taxRate="$taxRate" :roomBookings="$roomBookings ?? collect()" />

                <!-- Invoice Information -->
                @if ($hasInvoice)
                    <x-ecommerce.invoice-section :payment="$payment" :order="$order"
                        wrapperClass="mt-6 bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden" />
                @endif
            </div>
        </div>
    @else
