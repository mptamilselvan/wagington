    ...
    @if ($orderDetail)
        ...
        <div class="bg-gray-50 rounded-lg p-6 mb-6">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <p class="font-medium">
                        {{ \Carbon\Carbon::parse($orderDetail['order']['placed_at'])->format('d F Y') }}</p>
                </div>
                <div>
            ...
            @foreach ($orderDetail['items'] as $item)
                <div class="border border-gray-200 rounded-lg p-6">
                    ...
                    <div class="flex items-start mb-4">
                        {{-- Product Image --}}
                        <div
                            class="w-20 h-20 bg-gray-200 rounded-lg flex items-center justify-center mr-4 flex-shrink-0">
                            @if ($item['image_url'])
                        ...
                        <div class="text-right">
                            ...
                            @if ($orderDetail['order']['estimated_delivery'])
                                <p class="text-sm text-gray-600 mt-1">
                                    Arriving on
                                    {{ \Carbon\Carbon::parse($orderDetail['order']['estimated_delivery'])->format('jS F, Y') }}
                                </p>
                            @endif
                        </div>
                    </div>

                    {{-- Progress Steps - Hide for room bookings (catalog 3) --}}
                    @if (empty($item['is_room_booking']))
                        <div class="mb-4">
                            @php $steps = $this->getItemProgressSteps($item['fulfillment_status'] ?? 'pending'); @endphp
                            <div class="flex items-center justify-between">
                                @foreach ($steps as $index => $step)
                                    <div class="flex flex-col items-center flex-1">
                                        {{-- Step Icon --}}
                                        <div
                                            class="w-10 h-10 rounded-full flex items-center justify-center {{ $step['active'] ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-400' }}">
                                            @switch($step['icon'])
                                                @case('clock')
                                        ...
                                        {{-- Step Label --}}
                                        <p
                                            class="text-xs text-center mt-2 {{ $step['active'] ? 'text-blue-600 font-medium' : 'text-gray-500' }}">
                                            {{ $step['label'] }}
                                        </p>
                                    </div>
                                    ...
                                    @if ($index < count($steps) - 1)
                                        <div
                                            class="flex-1 h-0.5 mx-4 {{ $steps[$index + 1]['active'] ? 'bg-blue-600' : 'bg-gray-200' }}">
                                        </div>
                                    @endif
                                @endforeach
                            </div>
                    ...
                    {{-- Addons - Show status label only, no progress bar --}}
                                ...
                                <div class="flex items-center justify-between text-sm mb-3">
                                    <div class="flex items-center gap-3 flex-1">
                                        @if (!empty($addon['image_url']))
                                            <img src="{{ $addon['image_url'] }}" class="w-10 h-10 rounded object-cover"
                                                alt="{{ $addon['name'] }}" />
                                        @else
                                        ...
                                        <div class="flex-1">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <span class="text-gray-500">(Qty: {{ $addon['qty'] }})</span>
                                            </div>
                                            @php
                                                $addonStatus = $addon['fulfillment_status'] ?? 'pending';
                                                $statusColors = match ($addonStatus) {
                                                    'awaiting_stock' => 'bg-red-100 text-red-800',
                                                    'processing' => 'bg-yellow-100 text-yellow-800',
                                                    'shipped' => 'bg-blue-100 text-blue-800',
                                                    'delivered' => 'bg-green-100 text-green-800',
                                                    'awaiting_handover' => 'bg-purple-100 text-purple-800',
                                                    'handed_over' => 'bg-green-100 text-green-800',
                                                    default => 'bg-gray-100 text-gray-800',
                                                };
                                            @endphp
                                            <span
                                                class="inline-block mt-1 px-2 py-0.5 rounded-full text-xs font-medium {{ $statusColors }}">
                                                {{ $this->getStatusLabel($addonStatus) }}
                                            </span>
                                        </div>
                                    </div>
                                    <span
                                        class="text-gray-900 font-medium">${{ number_format($addon['total_price'], 2) }}</span>
                                </div>
                            @endforeach
                        </div>
                    @endif
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                @if ($orderDetail['addresses']['billing'])
                    <div class="mb-4">
                        ...
                        <div class="text-sm text-gray-600">
                            ...
                            <p>{{ $orderDetail['addresses']['billing']['first_name'] }}
                                {{ $orderDetail['addresses']['billing']['last_name'] }}</p>
                            ...
                            @endif
                            <p>{{ $orderDetail['addresses']['billing']['city'] }},
                                {{ $orderDetail['addresses']['billing']['postal_code'] }}</p>
                        </div>
                    </div>
                @endif
                ...
                @if ($orderDetail['addresses']['shipping'])
                    <div>
                        ...
                        <div class="text-sm text-gray-600">
                            ...
                            <p>{{ $orderDetail['addresses']['shipping']['first_name'] }}
                                {{ $orderDetail['addresses']['shipping']['last_name'] }}</p>
                            ...
                            @endif
                            <p>{{ $orderDetail['addresses']['shipping']['city'] }},
                                {{ $orderDetail['addresses']['shipping']['postal_code'] }}</p>
                        </div>
                    </div>
                @endif
            </div>
            ...
            <div class="bg-gray-50 rounded-lg p-6">
                ...
                <div class="space-y-2 mb-4">
                    ...
                    @if ($orderDetail['order']['shipping_amount'] > 0)
                        <div class="flex justify-between text-sm pt-2 border-t">
                            ...
                            <span
                                class="text-gray-900">${{ number_format($orderDetail['order']['shipping_amount'], 2) }}</span>
                        </div>
                    @endif
                    ...
                    @if ($orderDetail['order']['tax_amount'] > 0)
                        <div class="flex justify-between text-sm">
                            ...
                            <span
                                class="text-gray-900">${{ number_format($orderDetail['order']['tax_amount'], 2) }}</span>
                        </div>
                    @endif
                    ...
                    @if ($orderDetail['order']['discount_amount'] > 0)
                        <div class="flex justify-between text-sm">
                            ...
                            <span
                                class="text-green-600">-${{ number_format($orderDetail['order']['discount_amount'], 2) }}</span>
                        </div>
                    @endif
                </div>
                ...
                @if ($this->invoiceData)
                    @php
                        $payment = $this->invoiceData['payment'];
                        $orderNumber = $this->invoiceData['order_number'];
                    @endphp
                    <div class="mt-4 pt-4 border-t border-gray-200">
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center gap-3">
                            ...
                            @if ($payment['invoice_pdf_url'] && $orderNumber)
                                <a href="{{ route('customer.invoice.download', ['orderNumber' => $orderNumber, 'paymentId' => $payment['id']]) }}"
