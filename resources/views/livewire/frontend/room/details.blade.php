@section('meta_tags')
    ...
    <x-seo.meta-tags :title="$metaTitle" :description="$metaDescription" :canonicalUrl="$canonicalUrl" :type="'room'" />
@endsection
            ...
            <div class="mt-4 sm:mt-0 flex items-center gap-4">
                @if (!empty($selectedPrice))
                    <div class="text-2xl sm:text-3xl font-extrabold text-gray-900">
                        <span
                            class="text-base font-semibold tracking-wide mr-2">SGD</span>${{ number_format((float) $selectedPrice, 0) }}<span
                            class="text-xl font-bold text-gray-700">/day</span>
                    </div>
                @endif
                <button wire:click="openBooking"
                    class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-8 rounded-full shadow-lg transition-all duration-200 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
                    Book Now
                </button>
            </div>
        </div>
                ...
                <nav class="-mb-px flex space-x-8">
                    ...
                    @if ($hasHighlights)
                        <button @click="activeTab = 'highlights'"
                            :class="activeTab === 'highlights' ? 'border-blue-500 text-blue-600' :
                                'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'"
                            class="whitespace-nowrap py-2 px-1 border-b-2 font-medium text-lg">
                            Highlights
                        </button>
                    ...
                    @if ($hasTerms)
                        <button @click="activeTab = 'terms'"
                            :class="activeTab === 'terms' ? 'border-blue-500 text-blue-600' :
                                'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'"
                            class="whitespace-nowrap py-2 px-1 border-b-2 font-medium text-lg">
                            Terms & Conditions
                        </button>
                            ...
                            <div class="mt-8">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    @foreach ($roomType->room_attributes as $attribute)
                                        @if (!empty($attribute))
                                            <span
                                                class="inline-flex items-center px-3 py-1.5 bg-blue-100 text-blue-800 rounded-lg border border-blue-200 text-sm font-medium">{{ $attribute }}</span>
                                        @endif
                                    @endforeach
                                </div>
                            </div>
                            ...
                            <div class="mt-6">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    @foreach ($roomType->room_amenities as $amenity)
                                        @if (!empty($amenity))
                                            <span
                                                class="inline-flex items-center px-3 py-1.5 bg-green-100 text-green-800 rounded-lg border border-green-200 text-sm font-medium">{{ $amenity }}</span>
                                        @endif
                                    @endforeach
                                </div>
                            </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            @if (count($otherImages) > 0)
                                @php
                                    ...
                                    $individualImageHeight =
                                        $totalOtherImages > 0 ? $availableHeight / $totalOtherImages : 0;
                                @endphp
                                <div class="md:col-span-1 space-y-2">
                                    @foreach ($otherImages as $index => $image)
                                        <div style="height: {{ $individualImageHeight }}px; border-radius: 20px; overflow: hidden;"
                            ...
                            @if ($primaryImage)
                                <div class="md:col-span-2" style="border-radius: 20px; overflow: hidden;">
                                    <img src="{{ $primaryImage['url'] ?? $primaryImage }}"
                    ...
                    <div x-ref="relatedRail" class="flex gap-6 overflow-x-auto no-scrollbar scroll-smooth pb-2">
                        @foreach ($relatedRooms as $relatedRoom)
                            ...
                            <div class="w-64 flex-shrink-0">
                                <div
                                    class="group bg-white border rounded-[20px] overflow-hidden hover:shadow-lg transition-all duration-300 hover:border-blue-500 h-96">
                                    <!-- Room image -->
                                    <a href="{{ $relatedUrl }}"
                                        class="relative bg-gray-50 overflow-hidden flex items-center justify-center h-56 w-full">
                                        @if ($relatedRoom->getPrimaryImageUrl())
                                            <img src="{{ $relatedRoom->getPrimaryImageUrl() }}"
                                                alt="{{ $relatedRoom->name }}" class="object-cover w-full h-full" />
                                            <!-- Price overlay -->
                                            @if ($relatedRoom->getFormattedPrice())
                                                <div
                                                    class="absolute bottom-6 left-3 bg-white bg-opacity-95 rounded-[20px] px-6 py-4 text-center shadow-sm">
                                                    <div class="text-sm font-semibold text-blue-600">From
                                                        {{ $relatedRoom->getFormattedPrice() }}</div>
                                                </div>
                                            @endif
                                        @else
                                    ...
                                    <div class="flex flex-col justify-between h-[180px] p-5">
                                        <div class="flex-1">
                                            <div class="text-sm font-medium text-gray-900 line-clamp-2">
                                                {{ $relatedRoom->name }}</div>
                                            @if ($relatedRoom->room_description)
                                                <div class="mt-2 text-xs text-gray-600 line-clamp-3">
                                                    {{ \Illuminate\Support\Str::limit($relatedRoom->room_description, 150) }}
                                                </div>
                                            @endif
                                        </div>
