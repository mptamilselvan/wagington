<div x-data="{ open:false }" x-on:open-booking.window="open=true; $wire.refreshBooking(); if($event.detail.roomId){ $wire.setRoom($event.detail.roomId); } if($event.detail.roomTypeId){ $wire.room_type_id = $event.detail.roomTypeId; }" x-cloak>
    ...
    <!-- Panel -->
    <div x-show="open" class="fixed right-0 top-0 bottom-0 z-[61] w-full max-w-xl bg-white shadow-xl flex flex-col p-[20px] overflow-hidden h-full">
        ...
        <div class="bg-white px-4 pt-4 pb-1 space-y-4 border-b border-gray-100">
            ...
            <div class="bg-gray-50 border border-gray-200 rounded-2xl px-6 py-4 flex items-center justify-between">
                <div class="text-lg font-semibold text-gray-800">Total Add Ons Amount : S$ {{ number_format($this->addonsTotal, 2) }}</div>
        ...
        <div class="px-4 pt-4 pb-4 border-b border-gray-200">
            ...
            <div class="mt-6">
                ...
                <div wire:loading wire:target="checkAvailability" class="mt-2 text-sm text-gray-600 inline-flex items-center gap-2">
            ...
            @if($availabilityMessage)
                <div class="mt-3 text-sm rounded-lg border px-4 py-3 flex items-start gap-2 {{ $availabilityType === 'success' ? 'bg-green-50 border-green-200 text-green-800' : 'bg-red-50 border-red-200 text-red-800' }}">
                    @if($availabilityType === 'success')
                        <svg class="w-5 h-5 mt-0.5 text-green-600" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4A1 1 0 014.707 9.293L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path></svg>
                    @else
                        <svg class="w-5 h-5 mt-0.5 text-red-600" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10A8 8 0 11.001 10 8 8 0 0118 10zm-8-4a1 1 0 00-.894.553L8 7v4l1.106 2.447A1 1 0 0010 14h0a1 1 0 00.894-.553L12 11V7l-1.106-2.447A1 1 0 0010 6z" clip-rule="evenodd"/></svg>
                    @endif
                    <div class="flex-1">
            ...
            @if($availabilityType === 'success' && !empty($quote))
                <div class="mt-4 rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                    ...
                    <div class="p-6 space-y-4">
                        <div class="flex items-center justify-between">
                            <div class="text-xl font-semibold text-gray-900">{{ $quote['room_name'] ?? 'Room' }}</div>
                            <div class="text-xl font-semibold text-gray-900">${{ number_format($quote['final_subtotal'] ?? 0, 2) }}</div>
                        </div>
                        <div class="grid gap-2 text-sm text-gray-800">
                            <div>Pet Quantity: <span class="font-medium">{{ $quote['pet_quantity'] ?? 0 }}</span></div>
                            ...
                            <div class="mt-2">
                                ...
                                <div class="space-y-2">
                                    @foreach($quote['pet_lines'] as $pl)
                                        <div class="flex items-center justify-between">
                                            <div>
                                                <div class="font-medium">{{ $pl['pet_name'] ?? 'Pet' }} ({{ $pl['pet_size_name'] ?? 'Size' }})</div>
                                                <div class="text-xs text-gray-600">Base: ${{ number_format($pl['base_price'] ?? 0, 2) }} @if(($pl['variation_percent'] ?? 0) > 0) • Variation {{ number_format($pl['variation_percent'], 2) }}% = ${{ number_format($pl['variation_amount'] ?? 0, 2) }} @endif</div>
                                            </div>
                                            <div class="font-semibold">${{ number_format($pl['final_price'] ?? 0, 2) }}</div>
                                        </div>
                                    @endforeach
                                </div>
                            </div>
                        ...
                        <div class="space-y-1 text-sm">
                            <div class="flex items-center justify-between"><span>Room subtotal</span><span>${{ number_format($quote['base_total'] ?? 0, 2) }}</span></div>
                            @if(($quote['variation_total'] ?? 0) > 0)
                                <div class="flex items-center justify-between"><span>Variation</span><span>+ ${{ number_format($quote['variation_total'], 2) }}</span></div>
                            @endif
                            <div class="flex items-center justify-between"><span>Add-ons</span><span>${{ number_format($quote['addons_total'] ?? 0, 2) }}</span></div>
                        </div>
                    ...
                    <div class="space-y-2">
                        @foreach($aggreed_terms as $i => $term)
                            <label class="flex items-center gap-3 text-gray-700">
                                <input type="checkbox" class="w-5 h-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500" wire:model="agreeDocs.{{ $i }}">
                                <span>
                                    @if(!empty($term['content']))
                                        {{ $term['content'] }}
                                    @endif
                                    @if(!empty($term['document_url']))
                                        — <a href="{{ $term['document_url'] }}" target="_blank" class="text-blue-600 underline">View document</a>
                                    @endif
                                </span>
                            </label>
                        @endforeach
