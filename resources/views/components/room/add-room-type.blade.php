   ...
   <div class="grid grid-cols-1 gap-6 mb-6 md:grid-cols-2">
    ...
    <div class="grid grid-cols-1 gap-6 mb-6 md:grid-cols-2">
            ...
            <div>
                ...
                <!-- Image Previews -->
                @if(isset($imagePreviews) && is_array($imagePreviews) && count($imagePreviews) > 0)
                    <div class="mt-4">
                        <h4 class="text-sm font-medium text-gray-700 mb-2">Selected Images:</h4>
                        <div class="grid grid-cols-2 gap-4">
                            @foreach($imagePreviews as $index => $preview)
                                <div class="relative group" wire:key="image-preview-{{ $index }}">
                                    <img 
                                        src="{{ $preview }}" 
                                        alt="Preview {{ $index + 1 }}"
            ...
            <div>
                ...
                <select wire:model.live="selected_addon_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    <option value="">Select addon to add</option>
                    @foreach($serviceAddons as $serviceAddon)
                        @php
                            $isSelected = !empty($service_addons) && collect($service_addons)->contains('id', $serviceAddon->id);
                        @endphp
                        @if(!$isSelected)
                ...
                @if(!empty($service_addons) && count($service_addons) > 0)
                    <div class="mt-3 flex flex-wrap gap-2">
                        @foreach($service_addons as $index => $addon)
                            <div class="inline-flex items-center gap-2 px-3 py-1.5 bg-blue-100 text-blue-800 rounded-lg border border-blue-200" wire:key="addon-{{ $index }}">
    ...
    <div class="bg-gray-50 border border-gray-200 rounded-xl shadow-sm px-4 py-4 mb-6">
        <div class="flex items-center justify-between">
                    <h3 class="text-lg font-medium text-gray-900">Agreed Terms</h3>
                    <button 
                        type="button" 
                        wire:click="addAgreedTerm"
                class="px-4 py-2 text-blue-600 hover:text-blue-800  rounded-lg hover:bg-blue-50 flex items-center gap-2"
                    >
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                        </svg>
        ...
        <div class="grid grid-cols-1 gap-6 mb-6 md:grid-cols-2">
            ...
            <div>
                <label class="block mb-2 text-sm font-normal text-gray-700">Default Clean Minutes</label>
                <input 
                    type="text" 
            ...
            <div>
                <label class="block mb-2 text-sm font-normal text-gray-700">Turnover Buffer Min</label>
                <input 
                    type="text" 
    ...
    <div class="flex justify-end gap-4 mt-8 pt-6 border-t border-gray-200">
        ...
        <button 
            type="button" 
            wire:click="saveRoomType"
            wire:loading.attr="disabled"
            wire:loading.class="opacity-50 cursor-not-allowed"
            class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
        >
            <span wire:loading.remove wire:target="saveRoomType">
                Save Room Type
            </span>
            <span wire:loading wire:target="saveRoomType" class="flex items-center">
