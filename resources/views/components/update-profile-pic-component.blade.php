...
<div x-data="{ isPhoto: false }">

    @if( !empty($errors->photoUpdate->get('photo')) )
    <div x-data="{ isPhoto: true }">
        @endif

        <div @click="isPhoto=!isPhoto">
            {{$button}}
        </div>



        <div x-show="isPhoto" class="relative z-50" role="dialog" aria-modal="true">
            <!-- opacity -->
                    ...
                    max-w-[556px]">
                        <div
                            ...
                            <div
                                ...
                                <button @click="isPhoto =!isPhoto">
                                    <x-icons.close class="w-[20px] h-[24px] text-white" />
                                </button>
                            </div>

                            <div class="w-full h-full ">
                                @if (isset($update_Photo))
                                <div class="w-full h-full ">
                                    {{ $update_Photo}}
                                </div>
                                @endif
